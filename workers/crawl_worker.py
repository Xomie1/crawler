...
def crawl_url_job(url, company_name=None, use_ai=True, ai_provider='groq'):
    ...
    try:
        ...
        # Persist crawl result to DB (Phase 1 unified persistence)
        try:
            db = FormSubmissionDB()
            db.log_crawl_result(
                ...
                inquiry_form_url=result.get('inquiryFormUrl'),
                http_status=result.get('httpStatus'),
                robots_allowed=result.get('robotsAllowed'),
                crawl_status=result.get('crawlStatus'),
                error_message=result.get('errorMessage'),
                form_detection_method=result.get('formDetectionMethod'),
            )
            db.close()
        except Exception as e:
            logger.error(f"[{job_id}] Failed to persist crawl result: {e}")
